...
with header:
    ...
    with open('stockstyle.css') as f: 
         style= f.read()
    headers=["Historical Data", "Chart","Fundamentals", "Sentiments","Predict","About us"]
    
    tab1, tab2,tab6, tab3,tab4,tab5 = st.tabs(headers)
    ...
    with tab1:
            ...
            try:
                ...
                stock_data = hist_data.get('Time Series (Daily)') 
                ...
                if stock_data:
                    ...
                    s=stock_data.style.set_properties(**{'background-color': 'transparent',
                                'color': 'black',
                                'border-color': 'black'})
                    st.markdown("<p class=parag>Historical data upto yesterday</p> "
                                f"<style>{style}</style>"
                    ...
                    with st.spinner('Please wait...'):
                        time.sleep(10)     
                    st.dataframe(s,5000,use_container_width=True)
                    
                else:
                    hist_data=hist_data.get('Information')
                    hist_data
                    loaded =False
            except ConnectionError as e:
                error = "You need to check your internet connection"
                error 
                loaded=False
                hist_data ="There's a problem with your internet connection.Please connect to an active network"
    ...
    with tab3:
        ...
        try:
            ...
            if news_items:
                ...
                st.dataframe(user_view,use_container_width= True)
            else:
                loaded =False
        except ConnectionError as e:
            error = "You need to check your internet connection"
...
with contents:
            if loaded and conn:
                ...
                bals_df['fiscalDateEnding']=pd.to_datetime(bals_df['fiscalDateEnding'])
                ...
                display =fd_data[selected_columns]
                ...
                if conn:
                    text = "Sorry you have reached you daily API request limit. Please try again tomorrow or subscribe to Premium."
                    text
                else:
                    text =""
                text       
...
with tab4:
        ...
        if predictbtn:
            if loaded:
                ...
                st.markdown(f"<p class=val>Input Data </p> "
                            f"<style>{style}</style>"
                            ,unsafe_allow_html=True )
                model_input
                ...
                estHigh= prediction_df.iloc[0,0]
                estLow = prediction_df.iloc[0,1]
                if math.isnan(estHigh) and math.isnan(estLow):
                    st.markdown(f"<p class=val>The quarterly reports for this Symbol are yet to be released </p> "
                                f"<style>{style}</style>"
                               ,unsafe_allow_html=True )
                    tb=pd.concat([model_input.shift(periods=1),prediction_df],axis=1)
                    st.dataframe(tb,3000,use_container_width =True, hide_index= True)
                else: 
                       
                    st.markdown(f"<p class=val>Todays High will be :{estHigh}  </p> "
                                f"<p class=val>Todays Low will be : {estLow}</p> "
                                f"<style>{style}</style>"
                                ,unsafe_allow_html=True )
                ...
                message
with tab5:
    st.markdown(f"<p class=About>We are the number one and the best when it comes to predicting the markets.</p>"
                f"<p class=About>Our systems use AI and analyze the most important aspects of the stock market</p> "
                f"<p class=About>join a million others and say bye bye to red figures in your trading platform.</p> "
                f"<style>{style}</style>"
